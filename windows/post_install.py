    ...
    for dep in dll_list:
        if os.path.exists(BIN_DIR):
            if os.path.exists(os.path.join(BASE_PATH, "bin", dep)):
                shutil.copy(os.path.join(BASE_PATH, "bin", dep), os.path.join(BIN_DIR, dep))
...
def copy_gdk_pixbuff():
    ...
    copytree(
        os.path.join(BASE_PATH, "lib", "gdk-pixbuf-2.0"),
        gdk_pix_path
    )
    copytree(
...
def install_files(files:dict[Any], path: str) -> str:
    result = ""
...
def uninstall_files(content: dict[Any]) -> tuple[str]:
    files = ""
    dirs = ""
...
def main():
    ...
    clear_deps_search(
        ...
        INSTALL_PREFIX+"/bin/com.github.sidecuter.gciphers.exe",
        8
    )
